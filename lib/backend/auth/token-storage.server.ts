...
export async function getServerAuthTokens(): Promise<ServerAuthTokens> {
  ...
  return {
    accessToken:
      cookieStore.get(env.BACKEND_ACCESS_TOKEN_COOKIE_NAME)?.value ?? null,
    refreshToken:
      cookieStore.get(env.BACKEND_REFRESH_TOKEN_COOKIE_NAME)?.value ?? null,
  };
}
...
export async function persistServerAuthTokens(tokens: BackendTokenPair) {
  ...
  cookieStore.set(env.BACKEND_ACCESS_TOKEN_COOKIE_NAME, tokens.accessToken, {
    ...baseCookieOptions,
    maxAge:
      tokens.expiresIn || BACKEND_DEFAULT_ACCESS_TOKEN_COOKIE_MAX_AGE_SECONDS,
  });

  if (tokens.refreshToken) {
    cookieStore.set(
      env.BACKEND_REFRESH_TOKEN_COOKIE_NAME,
      tokens.refreshToken,
      {
        ...baseCookieOptions,
        maxAge:
